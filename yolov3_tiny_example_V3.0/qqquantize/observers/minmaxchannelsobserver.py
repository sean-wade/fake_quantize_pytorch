...
这是 风兴 的量化代码 里几个 Observer 类的实现，需要注意的事项有：
    1、功能及使用：
        a. MinMaxChannelsObserver(minmax统计-perchannel):
            最基础的observer，记录本层各channel的历史 min_value/max_value，
            计算各channel的scale

        b. MovingAverageMinMaxChannelsObserver(移动平均统计-perchannel):
            基于 MinMaxChannelsObserver 加入移动平均方法

        c. MinMaxActChannelsObserver(激活层专用的 perchannel observer)
            和 MinMaxChannelsObserver 的区别，也是 c_dim 不同
        
        d. MovingAverageMinMaxActChannelsObserver(移动平均统计-perchannel-act):
            基于 MinMaxActChannelsObserver 加入移动平均方法
    
    2、注意：
        使用时，请注意 act 层要用带act的per-channel的observer
'''
import torch
import math
...
class MinMaxActChannelsObserver(ObserverBase):
    ...
    def forward(self, x_orig):
        r"""Records the running minimum and maximum of ``x``."""
#         print("#="*10, "x_orig: ", x_orig.shape, type(x_orig))
        x = x_orig.detach()  # avoid keeping autograd tape
        x = torch.Tensor(x.to("cpu")).to(x_orig.device)  # add by zhanghao, convert QTensor(maybe) to torch.Tensor
        x = x.to(self.min_val.dtype)
        permute_idx = list(range(len(x.shape)))
        permute_idx[0] = self.c_dim
        permute_idx[self.c_dim] = 0
