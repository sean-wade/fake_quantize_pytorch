...
# add by zhanghao     
class FixMaxBiasObserver(ObserverBase):
    def __init__(self, max_bits = 12, bits=16, max_factor=1.0, device="cuda"):
        super().__init__()
        self.register_buffer('bits', torch.tensor([bits], dtype=torch.int))
        self.register_buffer('max_factor', torch.tensor([max_factor]))
        self.register_buffer('min_val', torch.tensor([]))
        self.register_buffer('max_val', torch.tensor([]))
        self.register_buffer('max_bits', torch.tensor([max_bits], device=device))
        self.qmin = - 2 ** (bits-1)
        self.qmax = 2 ** (bits-1) - 1
...
#             print("fixmaxbiasobserver forward: ", min_val.device, x.device)
            min_val = torch.min(torch.min(x), min_val)
            max_val = torch.max(torch.max(x), max_val)
        self.min_val.resize_(min_val.shape)
        self.max_val.resize_(max_val.shape)
    ...
    def calculate_qparams(self):
        ...
        scale is the unit size
        zero_point is always 0"""
#         print("returning max_bits = ", self.max_bits)
        scale = 0.5 ** self.max_bits
        zero_point = 0
        if scale == 0:
            raise ValueError('scale is 0')
        return torch.tensor([scale]), torch.tensor([zero_point])
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        super(FixMaxBiasObserver, self)._save_to_state_dict(destination, prefix, keep_vars)
        destination[prefix + 'min_val'] = self.min_val
        destination[prefix + 'max_val'] = self.max_val
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(FixMaxBiasObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict,
                                                          missing_keys, unexpected_keys, error_msgs)