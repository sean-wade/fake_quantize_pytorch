...
class QBatchNorm2d(nn.BatchNorm2d):
    ...
    def forward(self, input):
        ...
        if not self.training or self.track_running_stats:
            ...
            mean = self.running_mean.new_zeros(self.running_mean.shape)
            var_sqrt = torch.sqrt(self.eps)
        gamma = self.weight
        beta = self.bias
        ...
        return self.act_quant(
            input * self.weight_quant(weight.view(*shape)) + self.bias_quant(bias.view(*shape))
        )
