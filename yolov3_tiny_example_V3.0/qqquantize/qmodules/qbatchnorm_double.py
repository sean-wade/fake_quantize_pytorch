...
class QBatchNorm2d_d(nn.BatchNorm2d):
    ...
    def forward(self, input):
        ...
        F.batch_norm(
            ...
            self.running_mean.float().type(torch.DoubleTensor) if not self.training or self.track_running_stats else None,
            self.running_var.float().type(torch.DoubleTensor) if not self.training or self.track_running_stats else None,
            self.weight.float().type(torch.DoubleTensor),
            self.bias.float().type(torch.DoubleTensor),
