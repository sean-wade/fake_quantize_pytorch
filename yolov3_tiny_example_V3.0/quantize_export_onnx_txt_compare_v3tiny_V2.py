...
这是 风兴 的量化模型导出 onnx/txt/npy 代码，需要注意的事项有：
    ...
    1、这个代码与 quantize_export_onnx_txt_compare_v3tiny_V1.py 的区别是：
        将保存的txt中的key值后面加上 _pt 或者 _pc，仅此而已！
        例如：
            conv20_pt: [1.0]      （表示pertensor，因此scale有1个）
            act_19_pc: [1.0, 2.0] （表示perchannel，因此scale有多个）
    2、其他：
        这个是与软件部（李普铭）口头约定的，可能后期还会修改
'''
import os
import onnx
import torch
...
if __name__ == '__main__':
    ...
    # from models.yolo import Concat
    # COMPARE_QAT_MODULE_MAPPING[Concat] = qm.QConcat    # if yolov3(tiny)
    ...
    # model.load_state_dict(torch.load("ccc/yolotiny_fix_8bit.pt"))
    ...
    def forward_hooker(name):
        def hook_func(module, input, output):
            if isinstance(module, (qm.QConv2d_d, 
                ...
                np.save(NPY_PATH + name + '_in.npy', 
                        input[0].detach().cpu().numpy().astype(np.double))
#                 print(name)
#                 if not os.path.exists(NPY_PATH + name + '_out.npy'):
    ...
    for name, module in model.named_modules():
        if "module.model.20.m.0.act_quant" == name or "module.model.20.m.1.act_quant" == name:
            module.disable_fake_quant()
    with torch.no_grad():
        output1_noact, output2_noact = model(input_img)
    output1_noact = output1_noact.numpy()
    output2_noact = output2_noact.numpy()        
    np.save("output1_noact.npy", output1_noact.astype(np.double))
    np.save("output2_noact.npy", output2_noact.astype(np.double))
    
#     print(hook_data2.keys())
#     model_20_m_1_act_quant = hook_data2["module.model.20.m.0"]['values'][0]
#     print(model_20_m_1_act_quant.shape)
#     print(np.sum(model_20_m_1_act_quant - output2))
#     model_20_m_0_act_quant = hook_data2["module.model.20.m.0.act_quant"]['values'][0]
#     model_20_m_1_act_quant_npy_path = "model_20_m_1_act_quant.npy"
